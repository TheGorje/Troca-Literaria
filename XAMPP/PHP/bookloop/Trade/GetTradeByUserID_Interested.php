<?php
    ...
    if (isset($data['interested_id'])) {
        ...
        $sql = "SELECT * FROM trades WHERE interested_id = ? 
        AND (status = 'pending_confirmation' OR status = 'completed')";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $interested_id);
        $stmt->execute();
        ...
        while ($trade = $result->fetch_assoc()) {
            ...
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("s", $trade['advertiser_id']);
            $stmt->execute();
